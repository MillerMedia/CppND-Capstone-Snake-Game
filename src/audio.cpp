...
{
    playAudio(filename, nullptr, 0, volume);
}

void playMusic(const char * filename, int volume)
{
    playAudio(filename, nullptr, 1, volume);
}

void playSoundFromMemory(Audio * audio, int volume)
{
    playAudio(nullptr, audio, 0, volume);
}

void playMusicFromMemory(Audio * audio, int volume)
{
    playAudio(nullptr, audio, 1, volume);
}
...
void initAudio(void)
{
    ...
    global->buffer = nullptr;
    global->next = nullptr;

    /* want.userdata = newAudio; */
    if((gDevice->device = SDL_OpenAudioDevice(nullptr, 0, &(gDevice->want), nullptr, SDL_AUDIO_ALLOW_CHANGES)) == 0)
    {
...
{
    auto * newAudio = (Audio *) calloc(1, sizeof(Audio));
    ...
    newAudio->next = nullptr;
    newAudio->loop = loop;
    newAudio->fade = 0;
    newAudio->free = 1;
    newAudio->volume = volume;

    if(SDL_LoadWAV(filename, &(newAudio->audio), &(newAudio->bufferTrue), &(newAudio->lengthTrue)) == nullptr)
    {
    ...
    (newAudio->audio).callback = nullptr;
    (newAudio->audio).userdata = nullptr;
